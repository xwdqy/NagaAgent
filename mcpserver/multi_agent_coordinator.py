...
class MultiAgentCoordinator:
    ...
    def _init_coordinator(self):
        ...
        try:
            # 获取任务规划器和执行器
            if hasattr(self.agent_manager, 'task_planner'):
                self.task_planner = self.agent_manager.task_planner
            if hasattr(self.agent_manager, 'task_executor'):
                self.task_executor = self.agent_manager.task_executor
        ...
        try:
            ...
            if agent.agent_id == "general_assistant":
                ...
                if self.task_planner:
                    ...
                    return {"error": "任务规划器不可用"}
            elif agent.agent_id == "tool_executor":
                if self.task_executor:
                    # 这里可以调用具体的工具执行逻辑
                    return {"message": "工具执行器处理中..."}
                else:
                    return {"error": "任务执行器不可用"}
            else:
                # 默认使用通用助手
                return await self.agent_manager.call_agent("default", query)
