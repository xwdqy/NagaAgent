...
from voice import ThreadSafeVoiceIntegration  # 导入线程安全的语音集成
import json
...
class ChatWindow(QWidget):
    def __init__(s):
        ...
        from voice.unified_voice_manager import UnifiedVoiceManager, VoiceMode
        ...
        if u:
            ...
            if s.self_game_enabled:
                ...
                try:
                    ...
                    # 根据配置决定是否请求TTS
                    from system.config import config
                    if config.system.voice_enabled and config.voice_realtime.voice_mode in ["hybrid", "end2end"]:
                    ...
                    if resp.status_code == 200:
                        ...
                        # 使用打字机效果显示响应
                        s._start_non_stream_typewriter(final_message)

                        # 如果配置启用了语音且有音频URL，播放音频
                        from system.config import config
                        if config.system.voice_enabled and config.voice_realtime.voice_mode in ["hybrid", "end2end"] and "audio_url" in result:
                            s._play_text_audio(result["audio_url"])
                    else:
                        s.progress_widget.stop_loading()
                        s.add_user_message("系统", f"❌ 博弈论调用失败: {resp.text}")
                except Exception as e:
                    s.progress_widget.stop_loading()
                    s.add_user_message("系统", f"❌ 博弈论调用错误: {str(e)}")
                return
            else:
                # 普通模式：统一使用流式接口（获得更好的响应体验）
                try:
                    api_url = "http://localhost:8000/chat/stream"
                    data = {"message": u, "stream": True, "use_self_game": False}

                    # 根据配置决定是否请求TTS（不依赖语音服务是否运行）
                    from system.config import config
                    if config.system.voice_enabled and config.voice_realtime.voice_mode in ["hybrid", "end2end"]:
                        data["return_audio"] = True
    ...
    def _play_text_audio(s, audio_url):
        """播放文字输入的TTS音频"""
        try:
            import tempfile
            import os
    ...
    def handle_streaming_response(s, resp):
        ...
        try:
            ...
            # 如果配置启用了语音且有音频URL，播放音频
            from system.config import config
            if config.system.voice_enabled and config.voice_realtime.voice_mode in ["hybrid", "end2end"] and audio_url:
                s._play_text_audio(audio_url)
...
结果: {result[:200]}{'...' if len(result) > 200 else ''}
    ...
    def start_voice_realtime(s):
        ...
        try:
            ...
            # 使用统一语音管理器启动
            from voice.unified_voice_manager import VoiceMode
