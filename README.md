...
### 模式选择
- **非流式模式（推荐）**: 支持MCP工具调用，适合复杂任务处理
- **流式模式**: 实时显示文本，适合简单对话
- 在`config.json`中设置`"stream_mode": false`启用非流式模式
...
- **GRAG知识图谱记忆系统**，基于Neo4j的五元组知识图谱，自动提取对话中的实体关系，支持记忆查询和管理
...
│   ├── quintuple_visualize_v2.py  # 图谱可视化（解耦版本）
│   ├── main.py                 # 独立运行入口
│   └── quintuples.json         # 五元组缓存
├── logs/                       # 日志（含历史txt对话）
│   ├── 2025-04-27.txt
│   ├── 2025-05-05.txt
│   └── ...
...
- 支持历史对话一键导入GRAG知识图谱记忆系统，兼容主题、分层、五元组等所有新特性
...
### MCP工具调用优化
- **统一非流式模式**: 为支持MCP工具调用，统一使用非流式处理模式
- **工具调用支持**: 确保LLM输出完成后能正确进行工具调用处理
- **配置驱动**: 通过`config.json`中的`stream_mode`控制流式/非流式模式
- **向后兼容**: 保留流式模式代码，可通过配置切换

### UI显示优化
- **实时文本显示**: 修复前端UI显示延迟问题，文本立即显示不再等待
- **流式响应优化**: 改进流式数据处理，确保每个chunk立即显示到UI
- **进度状态优化**: 移除"整理最终回复"状态，避免用户等待感
- **UI更新机制**: 添加强制UI更新，确保文本实时显示

### 重大重构：语音系统异步化
- **完全异步处理**: 重构语音集成系统，文本显示和音频播放完全分离
- **消除重复播放**: 移除流式分句和最终文本的重复播放问题
- **前端即时显示**: 前端立即显示文本，不再等待音频处理完成
- **音频文件管理优化**: 改进音频文件清理机制，避免播放时文件被删除
- **线程安全**: 使用标准Queue替代asyncio.Queue，提升稳定性

### 性能优化
- **五元组提取优化**: 减少超时时间从20秒到12秒，添加重试机制(最多3次)，避免阻塞主流程
- **音频播放优化**: 短文本直接播放，长文本智能分句，提升响应速度
- **错误处理增强**: 改进网络超时和API调用失败的处理机制

### 配置更新
- 新增五元组提取相关配置项：`extraction_timeout`、`extraction_retries`、`base_timeout`
- 优化音频播放队列处理逻辑
- 改进日志记录，便于问题排查

### 技术改进
- **UI响应性**: 使用线程池异步处理音频，确保UI不被阻塞
- **内存管理**: 优化音频文件生命周期管理
- **错误恢复**: 增强网络异常和文件系统错误的恢复能力
