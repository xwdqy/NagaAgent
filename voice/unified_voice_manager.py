    ...
    def start_voice(self, mode: VoiceMode = None, config_params: Dict = None):
        ...
        try:
            ...
            if mode == VoiceMode.LOCAL:
                # 本地FunASR模式
                from voice.local_voice_integration import LocalVoiceIntegration
                self.voice_integration = LocalVoiceIntegration(self.parent)
                mode_name = "本地语音（FunASR）"

            elif mode == VoiceMode.END_TO_END:
                # 端到端模式
                from voice import ThreadSafeVoiceIntegration
                self.voice_integration = ThreadSafeVoiceIntegration(self.parent)
                mode_name = "端到端语音（通义千问）"

            elif mode == VoiceMode.HYBRID:
                # 混合模式
                from voice.voice_hybrid_integration import HybridVoiceIntegration
                self.voice_integration = HybridVoiceIntegration(self.parent)
                mode_name = "混合模式（支持MCP）"
